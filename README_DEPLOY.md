...
## Troubleshooting: Response API bukan JSON valid

1. Buka URL Vercel berikut di browser:

   `https://PROJECT-KAMU.vercel.app/api/apps-script`

2. Hasil yang benar berupa JSON dan `configured` harus bernilai `true`.

3. Jika `configured` bernilai `false`, isi `APPS_SCRIPT_API_URL` di Vercel lalu redeploy.

4. Di Apps Script, jalankan:

   `testVercelApiBridge()`

5. Pastikan deployment Apps Script:
   - Execute as: Me
   - Who has access: Anyone
   - URL berakhir dengan `/exec`
   - Deployment memakai versi Code.gs terbaru yang memiliki `doPost(e)`

6. Setelah mengubah Apps Script, buka Deploy > Manage deployments > Edit > New version > Deploy.

7. Setelah mengubah Environment Variable Vercel, lakukan redeploy project Vercel.
